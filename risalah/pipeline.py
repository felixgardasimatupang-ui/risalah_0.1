...
from risalah.id_terms import correct_transcript
...
def stage_enhance(merged, extracted_text=None, doc_analysis=False):
    ...
    if extracted_text and extracted_text.get("all_text_combined", "").strip():
        ...
        if len(doc_text) > 500 and not doc_analysis:
...
def stage_generate_docx(
    ...
    if preview:
        ...
        acara = (
            (metadata or {}).get("acara", "risalah").replace(" ", "_") if metadata else "risalah"
        )
        docx_path = os.path.join(base, f"{acara}.docx")
        return generate_risalah(
            enhanced,
            metadata,
...
def main():
    ...
    parser.add_argument("--engine", choices=["whisper", "assemblyai"], default="whisper")
    ...
    parser.add_argument(
        "--nomor", default="_______________", help="Nomor dokumen (contoh: 001/RISALAH/DINAS/2026)"
    )
    parser.add_argument(
        "--klasifikasi",
        choices=["BIASA", "TERBATAS", "RAHASIA"],
        default="BIASA",
        help="Klasifikasi dokumen",
    )
    parser.add_argument(
        "--doc-analysis",
        action="store_true",
    ...
    parser.add_argument(
        "--overwrite",
        action="store_true",
        help="Timpa output yang sudah ada (default: skip jika sudah ada cache)",
    )
    parser.add_argument(
        "--skip",
        nargs="*",
    ...
    run_pipeline(
        ...
        args.klasifikasi,
        args.title,
        doc_analysis=args.doc_analysis,
